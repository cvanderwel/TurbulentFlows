All the data files necessary for the sample exercises:

- [ChannelData.txt](ChannelData.txt)

- [HITData.txt](HITData.txt)

- [HSFData.txt](HSFData.txt)

- [MixingLayerData.txt](MixingLayerData.txt)

- [NoiseSample.txt](NoiseSample.txt)

- [PipeData.txt](PipeData.txt)

- [PlumeData1.txt](PlumeData1.txt)

- [PlumeData2.txt](PlumeData2.txt)

- [RoughWallData.txt](RoughWallData.txt)

- [TBLData.txt](TBLData.txt)

- [TurbulenceSample.txt](TurbulenceSample.txt)

- [WakeData.txt](WakeData.txt)
